<template>
  <article id="notes">
        ...
        <section class="article-list container animate__animated animate__slideInUp">
            <ul>
                <li v-for="(item,index) in list" :key="index" class="article-card" v-if="list.length">
                    <nuxt-link tag="div" :to="item.router">
                        <img src="@/static/img/git.jpg" alt="git" v-if="article == 'git'">
                        <img src="@/static/img/vue.jpg" alt="git" v-if="article == 'vue'">
                        <img src="@/static/img/nodejs.jpg" alt="git" v-if="article == 'node'">
                        <img src="@/static/img/nuxt.jpg" alt="git" v-if="article == 'nuxt'">
                    </nuxt-link>
                    <div class="text">
                        <div class="title">{{ item.title }}</div>
                        <div class="bottom">
                            <nuxt-link tag="p" :to="item.router" class="time"><i class="iconfont icontime"></i>{{ item.update_time }}</nuxt-link>
                            <!-- <nuxt-link tag="p" :to="item.router" class="time"><i class="iconfont icontime"></i>{{ typeof item.update_time }}</nuxt-link> -->
                            <p class="class"><i class="iconfont iconmark"></i>{{ item.skill_tag }}</p>
                        </div>
                    </div>
                </li>
            </ul>
...
export default {
    ...
    data () {
        return {
            article: 'vue',
            list: [
                { router: '/notes/git/1_install', title: 'vue基础', time: '2020-07-10', class: '前端' }
            ]
        }
    },
    mounted () {
    ...
    methods: {
        async search () {
            let result = await article_search( 2,this.article )
            if ( result.data.code == 200 ) {
                result.data.data.forEach(item => {
                    item.router = `/notes/${ item.article_class }/${ item.id }`
...
#notes{
    ...
    section.notes{
        ...
        div.tags{
            margin-bottom: 20px;
            .vue{ background: #F5EEF8;}
            .nuxt{ background: #F9EBEA;}
            .node{ background: #D7BDE2;}
            .git{ background: #FEF9E7;}
